...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		...
		self.layer.cornerRadius = [UIScreen mainScreen].bounds.size.width * 0.01;
...
{
	if (!_deleteButton){
		_deleteButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.bounds) - TAG_CELL_BTN_W - TAG_CELL_PADDING - 3, CGRectGetMidY(self.bounds) - TAG_CELL_BTN_W/2 - 1, TAG_CELL_BTN_W + 10, TAG_CELL_BTN_W + 2)];
...
+(CGSize)preferredSizeWithTag:(NSString*)tag deleteButtonEnabled:(BOOL)deleteButtonEnabled
{
	...
	CGSize itemSize = CGSizeMake(tagW + 2 * TAG_CELL_PADDING + deleteButtonW + TAG_CELL_PADDING, TAG_CELL_H);
	return itemSize;
}
@end
