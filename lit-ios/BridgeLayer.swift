...
class BridgeLayer: CALayer {
    ...
    private func bridgePath() -> CGMutablePathRef{
        ...
        let ballsize : CGFloat = 55
        let height : CGFloat = 105
        //let rect = CGRect(x: self.frame.origin.x, y:(22+(height * (1-percent))), width: 60, height: percent * (height))
        let rect = CGRect(x: self.frame.width/2, y:(2+(height * (1-percent))), width: 60, height: percent * (height))
        let path = CGPathCreateMutable()
        let inset : CGFloat = (rect.size.width-ballsize)/CGFloat(2)

   
        
        let modifier : CGFloat = 20
        let p1 = CGPoint(x: inset,                               y: rect.origin.y+modifier)
...
//            let curveInset : CGFloat = 0
            ...
            let p2 = CGPoint(x: rect.size.width - inset ,            y: rect.origin.y+modifier)
            let p3 = CGPoint(x: rect.size.width - newCurveInset,     y: rect.size.height * 0.55 + rect.origin.y+modifier)
            let p4 = CGPoint(x: rect.size.width,                     y: rect.size.height + rect.origin.y+modifier)
            let p5 = CGPoint(x: 0,                                   y: rect.size.height + rect.origin.y+modifier)
            let p6 = CGPoint(x: newCurveInset,                       y: rect.size.height * 0.55 + rect.origin.y+modifier)
